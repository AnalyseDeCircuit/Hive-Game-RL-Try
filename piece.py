...
class Piece(ABC):
    def __init__(self, x: int, y: int, piece_type: int, owner):
        self.x = x
        self.y = y
    ...
    def set_piece_type(self, new_piece_type: int):
    ...
    @abstractmethod
    def is_valid_move(self, to_x: int, to_y: int, board) -> bool:
        pass
...
class QueenBee(Piece):
    ...
    def is_valid_move(self, to_x: int, to_y: int, board) -> bool:
        dx = abs(to_x - self.x)
        dy = abs(to_y - self.y)
        # QueenBee can move one step in any direction (including diagonals in this simplified model)
        # And the target position must be adjacent to an existing piece (connected hive rule)
        return (dx <= 1 and dy <= 1 and (dx + dy > 0)) and board.has_adjacent_piece(to_x, to_y)
...
class Beetle(Piece):
    ...
    def is_valid_move(self, to_x: int, to_y: int, board) -> bool:
        dx = abs(to_x - self.x)
        dy = abs(to_y - self.y)
        # Beetle can move one step in any direction (including diagonals in this simplified model)
        # It can also move on top of other pieces, so no need to check if target is empty.
        return (dx <= 1 and dy <= 1 and (dx + dy > 0)) and board.has_adjacent_piece(to_x, to_y)
...
class Pillbug(Piece):
    ...
    def is_valid_move(self, to_x: int, to_y: int, board) -> bool:
        # Pillbug moves one step, similar to QueenBee, but also has a special ability.
        # The C++ code uses Manhattan distance.
        dx = abs(to_x - self.x)
        dy = abs(to_y - self.y)
        return (dx <= 1 and dy <= 1 and (dx + dy > 0)) and board.has_adjacent_piece(to_x, to_y)
